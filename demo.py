...
# Position
# ===========================
axs[0].plot(frame_numbers, left_knee_y,
         color='lightcoral',
         linewidth=2,
         alpha = 0.4,
         label='Left Knee (Raw)')

axs[0].plot(frame_numbers, left_y_filtered,
         color='red',
         linewidth=2,
         label='Left Knee')

axs[0].plot(frame_numbers, right_knee_y,
         color='skyblue',
         linewidth=2,
         alpha=0.4,
         label='Right Knee (Raw)')

axs[0].plot(frame_numbers, right_y_filtered,
         color='blue',
         linewidth=2,
         label='Right Knee')
...
# Velocity Angle
# -----------------------------
left_angle = np.abs(np.degrees(np.arctan2(left_vy, left_vx)))
right_angle = np.abs(np.degrees(np.arctan2(right_vy, right_vx)))

left_velocity = np.sqrt(left_vx**2 + left_vy**2)
right_velocity = np.sqrt(right_vx**2 + right_vy**2)
...
for i in range(len(left_angle)):
    ...
    if flag:
        print(f"Left: Frame {frame_numbers[i]} Angle = {left_angle[i]}")
...
for i in range(len(right_angle)):
    ...
    if flag:
        print(f"Right: Frame {frame_numbers[i]} Angle = {right_angle[i]}")
