...
video_path = r"C:\Users\Arnav Ranjan\OneDrive\Desktop\GaitON Internship\Walking videos\New folder\1_left.mp4"
...
left_kx = np.abs(np.degrees(np.arctan(np.gradient(left_x_filtered, frame_numbers))))
right_kx = np.abs(np.degrees(np.arctan(np.gradient(right_x_filtered, frame_numbers))))
# -----------------------------
# Plot
# -----------------------------
...
#for i in range(len(frame_numbers)):
    #if left_kx[i] >=0:
        #axs[0].text(frame_numbers[i],left_x_filtered[i],'+',color="green",fontsize=11)
    #else:
        #axs[0].text(frame_numbers[i],left_x_filtered[i],'-',color="red",fontsize=11)
...
#for i in range(len(frame_numbers)):
    #if right_kx[i] >= 0:
        #axs[0].text(frame_numbers[i],right_x_filtered[i],'+',color="green",fontsize=11)
    #else:
        #axs[0].text(frame_numbers[i],right_x_filtered[i],'-',color="red",fontsize=11)

right = [27,89,151]
left = [58,120,182]

for a in right:
    axs[0].axvline(x=a, color='blue',linestyle="--",linewidth = 1)

for b in left:
    axs[0].axvline(x=b,color="red",linestyle="--",linewidth = 1)
...
for i in range(len(frame_numbers) - 1):
    left_kx_1.append(np.degrees(np.atan((left_x_filtered[i+1] - left_x_filtered[i]))))

right_kx_1 = []
for i in range(len(frame_numbers) - 1):
    left_kx_1.append(np.degrees(np.atan((right_x_filtered[i+1] - right_x_filtered[i] ))))


for i in range(len(frame_numbers)):
    print(f"{frame_numbers[i]:4d}\t{left_kx[i]:8.2f}\t\t{right_kx[i]:8.2f}")
...
while i < len(left_kx):
    ...
    while i < len(left_kx) and left_kx[i] <= 15:
        array.append(i)
        i += 1
...
while i < len(right_kx):
    array = []

    while i < len(right_kx) and right_kx[i] <= 15:
        array.append(i)
        i += 1
...
#left_vx_diff = np.gradient(left_vx, frame_numbers)

#for i in range(len(frame_numbers) - 1):
    #slope = left_vx_diff[i] / 1
    #print(i, np.degrees(np.arctan(slope)), slope)
