...
def main():
    ...
    # --- NEW: Auto-Zoom to completely ignore off-screen AI glitch spikes ---
    if len(timestamps) > 0:
        # Zoom Position Graph (Inverted Y-axis)
        y_all = np.concatenate([sr_heel[:, 1], sl_heel[:, 1]])
        axs[0].set_ylim(np.percentile(y_all, 95) + 50, np.percentile(y_all, 5) - 50)
        
        # Zoom Velocity Graph
        v_all = np.concatenate([vr_heel, vl_heel])
        v_max = np.percentile(v_all, 95) * 1.5
        axs[1].set_ylim(-10, v_max if v_max > 0 else 100)
        
        # Zoom Acceleration Graph
        a_all = np.concatenate([ar_heel, al_heel])
        a_max = np.percentile(np.abs(a_all), 95) * 1.5
        axs[2].set_ylim(-a_max if a_max > 0 else -100, a_max if a_max > 0 else 100)
    # -----------------------------------------------------------------------
